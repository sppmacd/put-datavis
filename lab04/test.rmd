...
category_colors <- reactive(setNames(
  ggsci::pal_aaas()(length(unique(category_data()$category_name))),
  unique(category_data()$category_name)
))

# here we will use these category colors
GGTHEME = reactive(
  labs(color=NULL, alpha=NULL, fill=NULL) +
...
```{r}
# And HERE, show legend, common for all plots.

renderUI({
  tags$div(
    style = "margin: 10px;",
    tags$h4("Category Colors"),
    tags$ul(
      lapply(names(category_colors()), function(cat) {
        tags$li(style = paste0("color: ", category_colors()[[cat]]), cat)
      })
    )
  )
})
